...
class PathOptimizer(Model):
    ...
    currency_set = (
        None  # a set of length n, where n is the number of currencies included.
    )
    ...
    withdrawal_fee = (
        None  # a dict to record the withdrawal fee of each exchange's currencies
    )
    include_fiat = None  # boolean variable, to decide whether the arbitrage path should include fiat currencies.
    inter_exchange_trading = (
        None  # boolean variable to determine whether considers inter exchange trading
    )
    ...
    var_location = (
        None  # an array that records the location of all the decision variables
    )
    ...
    balance_dict = (
        None  # a dict to record all the currencies' balances in all exchanges
    )
    price = None  # a nested dict that records the ticker information of all trading pairs in all exchanges
    inter_convert_list = (
        None  # a list that records all the feasible inter-exchange pairs
    )
    ...
    def find_arbitrage(self):
        ...
        self.print_content = "profit rate: {}, arbitrage path: {}".format(
            self.obj, self.path
        )
    ...
    def init_currency_info(self):
        ...
        for exc_name, exchange in self.exchanges.items():
            ...
            currency_names = [
                "{}_{}".format(exc_name, cur) for cur in exchange.currencies.keys()
            ]
            self.currency_set |= set(currency_names)
            if not self.include_fiat:
                self.currency_set -= set(
                    ["{}_{}".format(exc_name, fiat) for fiat in self.fiat_set]
                )
    ...
    def update_transit_price(self):
        ...
        exc_price_list = multiThread(
            self.parallel_fetch_tickers, exc_name_list, thread_num
        )
    ...
    def update_commission_fee(self):
        ...
        for exc_name in self.exchanges.keys():
            indexes = [
                index
                for cur_name, index in self.currency2index.items()
                if exc_name in cur_name
            ]
            self.commission_matrix[
                np.meshgrid(indexes, indexes, indexing="ij", sparse=True)
            ] = self.trading_fee[exc_name]
        ...
        for from_cur, to_cur in self.inter_convert_list:
            ...
            if from_cur in self.withdrawal_fee:
                self.commission_matrix[from_index, to_index] = self.withdrawal_fee[
                    from_cur
                ]["usd_rate"]
            if to_cur in self.withdrawal_fee:
                self.commission_matrix[to_index, from_index] = self.withdrawal_fee[
                    to_cur
                ]["usd_rate"]
    ...
    def set_constraints(self):
        ...
        self.add_constraints(
            self.sum(self.x[currency, :]) == self.sum(self.x[:, currency])
            for currency in range(self.length)
        )
        # 2. each currency can only be transited-in at most once.
        self.add_constraints(
            self.sum(self.x[currency, :]) <= 1 for currency in range(self.length)
        )
    ...
    def update_objectives(self):
        ...
        final_transit_matrix = np.log(
            self.transit_price_matrix
            * (1 - self.commission_matrix)
    ...
    def update_vol_matrix(self, percentile=0.01):
        ...
        for key, val in self.price.items():
            ...
            if (
                base_coin in self.crypto_prices
                and val["baseVolume"] is not None
                and self.crypto_prices[base_coin]["price"] is not None
            ):
                usd_values[key] = (
                    val["baseVolume"]
                    * self.crypto_prices[base_coin]["price"]
                    * percentile
                )
        ...
        for key, val in usd_values.items():
            ...
            if from_cur in self.currency_set and to_cur in self.currency_set:
                self.vol_matrix[
                    self.currency2index[from_cur], self.currency2index[to_cur]
                ] = val
                self.vol_matrix[
                    self.currency2index[to_cur], self.currency2index[from_cur]
                ] = val
        ...
        for from_cur, to_cur in self.inter_convert_list:
            ...
            if self.consider_inter_exc_bal:
                from_cur_bal = (
                    self.balance_dict[from_cur]["usd_balance"]
                    if from_cur in self.balance_dict
                    else 0
                )
                to_cur_bal = (
                    self.balance_dict[to_cur]["usd_balance"]
                    if to_cur in self.balance_dict
                    else 0
                )
            else:
            ...
            if from_cur in self.withdrawal_fee:
                ...
                self.vol_matrix[
                    self.currency2index[from_cur], self.currency2index[to_cur]
                ] = (to_cur_bal + from_cur_withdraw)
            if to_cur in self.withdrawal_fee:
                to_cur_withdraw = self.withdrawal_fee[to_cur]["usd_fee"]
                self.vol_matrix[
                    self.currency2index[to_cur], self.currency2index[from_cur]
                ] = (from_cur_bal + to_cur_withdraw)
    ...
    def parallel_fetch_tickers(self, exc_name):
        ...
        for pair in list(exc_price.keys()):
            ...
            if (
                len(match) == 1
                and match[0] == pair
                and pair in self.exchanges[exc_name].markets
            ):
                new_name = "/".join(
                    ["{}_{}".format(exc_name, i) for i in pair.split("/")]
                )
    ...
    def update_changeable_constraint(self):
        ...
        if self.consider_init_bal:
            ...
            coin_balance_list = [
                (key, val["usd_balance"])
                for key, val in self.balance_dict.items()
                if val["usd_balance"] >= self.min_trading_limit
            ]
            coin_balance_list = sorted(
                coin_balance_list, key=lambda x: x[-1], reverse=True
            )
            ...
            if self.required_currencies == []:
                ...
                required_cur_index = [
                    self.currency2index[i] for i in self.required_currencies
                ]
